...
PlotBeachball() { 
   ...
   local _colorPen=$9
	...
	if [ `echo $_stk $_dip $_rak | awk '{if($1==0&&$2==0&&$3==0){print 0}else{print 1}}'` == 1 ]; then
		echo $_lon $_lat $_dep $_stk $_dip $_rak 5. 0 0 | psmeca -R -J -Sa0.6 -G$_colorFill -W1,$_colorPen -O -K >> $_psout
		echo $_lon $_lat | psxy -R -J -Sc0.65 -W5,$_colorPen -O -K >> $_psout
	else
		echo $_lon $_lat | psxy -R -J -Sc0.65 -G$_colorFill -W2,$_colorPen -O -K >> $_psout
	fi
	if [ $# -gt 9 ]; then
...
idir=0
lonavg=0
for rdir in `ls -d results_SAMC_?_1000_Ei results_SAMC_?_1000_1D results_SAMC_?_500_Ei results_SAMC_?_500_1D`; do
...
for (( idir=0; idir<$ndir; idir++ )); do
	...
	PlotBeachball $psout `echo ${model_best[idir]} | awk '{print $1,$2,$3,$4,$5,$6}'` $_colorP ${colorE[ilab]}
done
...
#EOF
echo $lonmid $latmid
pslegend -R -J -D${ulon}/${llat}/2.3/4.6/BR -G200 -F -O -K <<- EOF >> $psout
	G -0.15i
	M $lonmid $latmid 1 p
	G 0.12i
	S $dx1 a $size_a ${colorP[0]} $pen,${colorP[0]} 0.4i 3D
	G $gap
	S $dx1 a $size_a ${colorP[1]} $pen,${colorP[1]} 0.4i 1D
	G 0.1i
