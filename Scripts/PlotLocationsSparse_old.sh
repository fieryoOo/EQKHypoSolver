...
#for (( idir=0; idir<$ndir; idir++ )); do
	while read line; do
		...
		PlotBeachball $psout $model_best $_color #$_color $trlno
	done < $_fin
}

### main ###
echo "Usage: "$0" [plot region (optional)]"
if [ $# != 0 ] && [ $# != 1 ]; then
	exit
fi
...
# distance
dirtmp=`ls -d results_SAMC_B_*_*_Sparse1 | head -n1`
dis=`echo $dirtmp | cut -d_ -f4`
echo $dis
...
#for rdir in `ls -d results_SAMC_L_${dis}_1D_Sparse* results_SAMC_R_${dis}_1D_Sparse* results_SAMC_L_${dis}G_1D_Sparse* results_SAMC_B_${dis}_1D_Sparse* results_SAMC_L_${dis}_Ei_Sparse* results_SAMC_R_${dis}_Ei_Sparse* results_SAMC_L_${dis}G_Ei_Sparse* results_SAMC_B_${dis}_Ei_Sparse*`; do
#for rdir in `ls -d results_SAMC_L_${dis}_1D_Sparse* results_SAMC_R_${dis}_1D_Sparse* results_SAMC_B_${dis}_1D_Sparse*`; do
for rdir in `ls -d results_SAMC_L_${dis}_Ei_Sparse* results_SAMC_R_${dis}_Ei_Sparse* results_SAMC_B_${dis}_Ei_Sparse*`; do
#for rdir in `ls -d results_SAMC_L_${dis}_1D_Sparse? results_SAMC_R_${dis}_1D_Sparse? results_SAMC_L_${dis}G_1D_Sparse? results_SAMC_B_${dis}_1D_Sparse? results_SAMC_L_${dis}_Ei_Sparse? results_SAMC_R_${dis}_Ei_Sparse? results_SAMC_L_${dis}G_Ei_Sparse? results_SAMC_B_${dis}_Ei_Sparse?`; do
#for rdir in `ls -d results_SAMC_B_${dis}_Ei_Sparse*`; do

	rtype=`echo $rdir | awk -F/ '{print $NF}' | cut -d_ -f3,4,5,6`
...
if [ 1 == 1 ]; then
   ...
   if [ `echo $line | grep -c 'best'` != 1 ]; then
	...
	#echo $line | awk -F'minfo = \\(' '{print $2}' | awk -v rtype=${rtype} -F')' '{print $1,rtype}' >> $fres
	model_best=`echo $line | awk -F'(' '{print $3}' | cut -d')' -f1` # | awk '{print $1,$2,$3,$4,$5,$6,$7}'`
	echo ${model_best} ${rtype} >> $fres
	#cloc[idir]=`echo ${model_best} | awk '{print $5"_"$6}'`
...
#	G 0.03i
#	S $dx1 e $size_e - $pen,${colors[0]} $dx2 `echo ${rtype[0]} | cut -d_ -f1,3 | sed s/'Ei'/'3D'/ | sed s/'B'/'Both'/ | sed s/'R'/'Rayl'/`
#	G $gap
#	S $dx1 e $size_e - $pen,${colors[1]} $dx2 `echo ${rtype[1]} | cut -d_ -f1,3 | sed s/'Ei'/'3D'/ | sed s/'B'/'Both'/ | sed s/'R'/'Rayl'/`
#	G $gap
#	S $dx1 e $size_e - $pen,${colors[2]} $dx2 `echo ${rtype[2]} | cut -d_ -f1,3 | sed s/'Ei'/'3D'/ | sed s/'B'/'Both'/ | sed s/'R'/'Rayl'/`
#	G $gap
#	S $dx1 e $size_e - $pen,${colors[3]} $dx2 `echo ${rtype[3]} | cut -d_ -f1,3 | sed s/'Ei'/'3D'/ | sed s/'B'/'Both'/ | sed s/'R'/'Rayl'/`
#	G $gap
#EOF
echo $lonmid $latmid
