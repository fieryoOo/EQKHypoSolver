...
PlotHisto() {
   ...
   std=`awk -v cN=$columnN -v mean=$mean 'BEGIN{sum=0}{sum+=($cN-mean)**2}END{std=sqrt(sum/(NR-1));if(std<0.0001)std=0.0001; print std}' $h_infile`
...
awk 'BEGIN{flag=1;NFold=0}{if(NF==0&&NFold!=0){flag++} if(flag==1)print; NFold=NF}' $fin | grep "search#" | awk -v is=$is -v ie=$ie '$2>is&&$2<ie' | awk -F\( '{print $3,$1,$4}' | sed s/')'/''/g | awk -v Emul=$Emul -v is=$is '{if(NF==17){M0=$5}else{M0=1} printf "%d %f %d ",NR+is,$(NF-3)*Emul,$(NF-6); print $1,$2,$3,$4,M0,$(NF-11),$(NF-10),$(NF-9),$(NF);}' | awk -v cm=$chiS_max '$2/$3<cm' > .PlotPosterior_tmp
...
#echo .PlotPosterior_tmp .PlotPosterior_tmp_rej .PlotPosterior_tmp_acc
#exit
...
#rCl=`echo $rCrange | awk '{printf "%.2f", $1-0.2}'`
#rCu=`echo $rCl | awk '{printf "%.2f", $1+2.}'`
rCl=0; rCu=3; rCu2=30
#rCl=2.1; rCu=2.4
...
PlotHisto .PlotPosterior_tmp_acc 4 -13.4 -6.5 "strike" "strike (deg)" 0 360 1 >> results
PlotHisto .PlotPosterior_tmp_acc 5 6.7 0 "dip" "dip (deg)" 0 90 1 >> results
PlotHisto .PlotPosterior_tmp_acc 6 6.7 0 "rake" "rake (deg)" -180 180 1 >> results
PlotHisto .PlotPosterior_tmp_acc 7 -10.7 -6.2 "depth" "depth (km)" 0 60 1 >> results
PlotHisto .PlotPosterior_tmp_acc 8 8 0 "M0" "M0" 0. 1.e24 1.e22 >> results
