     ...
     +                       sfici, slami, seismz, seismn, seisme, rotate )
         ...
         real*4 slat, slon, sfici, slami, tstart, w, vmax, fix_vel
         real*4 elatc,elonc
         real*4 azi, azi_back, f0, f1, f2, f3, f4
...
C         write(*,*) ' DIST= ',dist,' AZI= ',fi,' BACK_AZI= ',fi_back
c ---
         cs=cos(AZI)
         sc=sin(AZI)
...
C----------INTERPOLATION OF SPECTRAL AMPLITUDES-------------S  
         if (sigR.eq.'+') then
           call calspecr(nt,nf,f0,df,fr,az,ah,sreV,simV,sreR,simR,f1,f2,f3,f4,iq)
         endif
         if (sigL.eq.'+') 
     +      call calspecl(nt,nf,f0,df,fr,al,sreT,simT,f1,f2,f3,f4,iq)
