...
c---------------------------------------------------------------------
c---------------------------------------------------------------------
C     +                   feig_buff,eiglen, phvnper,dperin,
C     +                   strike,dip,rake,depth, period,nper,
C     +                   azimuth,groupT,phaseT,amplitude)
      subroutine surfsyn(runfile, infile, outfile, wavetype, symbo, depth)
      implicit none
...
C         fname = 'sac/'//codi(mm)(1:lnblnk(codi(mm)))//'.BHE.SAC'
C           call GETARG(1,runfile)
C         runfile = 'RUNR'
C           call GETARG(2,infile)
C         infile = 'R.R'
           lin=lnblnk(infile)
C           call GETARG(3,outfile)
C         outfile = 'bred'
           nout=lnblnk(outfile)
C           call GETARG(4,wavetype)
C         wavetype = 'R'
C           call GETARG(5,symbo)
C         symbo = '0'
           read(symbo,*)mode
         write(*,*) "!",runfile,"!"
         write(*,*) "!",infile,"!"
...
C-----------its is type of source:C/quake-double dipole----------------
      PRINT*,'Wavetype=',wavetype,' mode=',mode
...
C ------------ write sac files ---------
        ...
        chn = 'BHZ'
C        call wsac(fnam11,elat,elon,codi(mm),figi(mm),
C     +            lami(mm),dt,chn,neti(mm),npoints,seismz)
        chn = 'BHN'
C        call wsac(fnam12,elat,elon,codi(mm),figi(mm),
C     +            lami(mm),dt,chn,neti(mm),npoints,seismn)
        chn = 'BHE'
C        call wsac(fnam13,elat,elon,codi(mm),figi(mm),
C     +            lami(mm),dt,chn,neti(mm),npoints,seisme)
